...
fn build_flight_dataframe(flights: &[FlightLegRecord<'_>]) -> PolarsResult<DataFrame> {
    ...
    let ((c1, c2), (c3, c4)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "flight_designator".into(),
                        flights
                            .iter()
                            .map(|r| r.flight_designator.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "operational_suffix".into(),
                        flights
                            .iter()
                            .map(|r| r.operational_suffix.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "airline_designator".into(),
                        flights
                            .iter()
                            .map(|r| r.airline_designator.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "control_duplicate_indicator".into(),
                        flights
                            .iter()
                            .map(|r| r.control_duplicate_indicator.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
    ...
    let ((c5, c6), (c7, c8)) = rayon::join(
            ...
            rayon::join(
                || {
                    Column::new(
                        ...
                        flights
                            .iter()
                            .map(|r| r.flight_number.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "itinerary_variation_identifier".into(),
                        flights
                            .iter()
                            .map(|r| r.itinerary_variation_identifier.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "leg_sequence_number".into(),
                        flights
                            .iter()
                            .map(|r| r.leg_sequence_number.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
    ...
    let ((c9, c10), (c11, c12)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "period_of_operation_from".into(),
                        flights
                            .iter()
                            .map(|r| r.period_of_operation_from.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "period_of_operation_to".into(),
                        flights
                            .iter()
                            .map(|r| r.period_of_operation_to.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "days_of_operation".into(),
                        flights
                            .iter()
                            .map(|r| r.days_of_operation.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
    ...
    let ((c13, c14), (c15, c16)) = rayon::join(
            ...
            rayon::join(
                || {
                    Column::new(
                        ...
                        flights
                            .iter()
                            .map(|r| r.departure_station.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "scheduled_time_of_passenger_departure".into(),
                        flights
                            .iter()
                            .map(|r| r.scheduled_time_of_passenger_departure.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "scheduled_time_of_aircraft_departure".into(),
                        flights
                            .iter()
                            .map(|r| r.scheduled_time_of_aircraft_departure.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "time_variation_departure".into(),
                        flights
                            .iter()
                            .map(|r| r.time_variation_departure.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
    ...
    let ((c17, c18), (c19, c20)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "passenger_terminal_departure".into(),
                        flights
                            .iter()
                            .map(|r| r.passenger_terminal_departure.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "arrival_station".into(),
                        flights
                            .iter()
                            .map(|r| r.arrival_station.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "scheduled_time_of_aircraft_arrival".into(),
                        flights
                            .iter()
                            .map(|r| r.scheduled_time_of_aircraft_arrival.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "scheduled_time_of_passenger_arrival".into(),
                        flights
                            .iter()
                            .map(|r| r.scheduled_time_of_passenger_arrival.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
    ...
    let ((c21, c22), (c23, c24)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "time_variation_arrival".into(),
                        flights
                            .iter()
                            .map(|r| r.time_variation_arrival.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "passenger_terminal_arrival".into(),
                        flights
                            .iter()
                            .map(|r| r.passenger_terminal_arrival.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
            ...
            rayon::join(
                || {
                    Column::new(
                        ...
                        flights
                            .iter()
                            .map(|r| r.aircraft_type.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "passenger_reservations_booking_designator".into(),
                        flights
                            .iter()
                            .map(|r| r.passenger_reservations_booking_designator.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
    ...
    let ((c25, c26), (c27, c28)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "passenger_reservations_booking_modifier".into(),
                        flights
                            .iter()
                            .map(|r| r.passenger_reservations_booking_modifier.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "meal_service_note".into(),
                        flights
                            .iter()
                            .map(|r| r.meal_service_note.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "joint_operation_airline_designators".into(),
                        flights
                            .iter()
                            .map(|r| r.joint_operation_airline_designators.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "min_connecting_time_status_departure".into(),
                        flights
                            .iter()
                            .map(|r| r.min_connecting_time_status_departure.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
    ...
    let ((c29, c30), (c31, c32)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "min_connecting_time_status_arrival".into(),
                        flights
                            .iter()
                            .map(|r| r.min_connecting_time_status_arrival.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "secure_flight_indicator".into(),
                        flights
                            .iter()
                            .map(|r| r.secure_flight_indicator.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "itinerary_variation_identifier_overflow".into(),
                        flights
                            .iter()
                            .map(|r| r.itinerary_variation_identifier_overflow.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
    ...
    let ((c33, c34), (c35, c36)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "cockpit_crew_employer".into(),
                        flights
                            .iter()
                            .map(|r| r.cockpit_crew_employer.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "cabin_crew_employer".into(),
                        flights
                            .iter()
                            .map(|r| r.cabin_crew_employer.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
            ...
            rayon::join(
                || {
                    Column::new(
                        ...
                        flights
                            .iter()
                            .map(|r| r.onward_flight.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "airline_designator2".into(),
                        flights
                            .iter()
                            .map(|r| r.airline_designator2.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
    ...
    let ((c37, c38), (c39, c40)) = rayon::join(
            ...
            rayon::join(
                || {
                    Column::new(
                        ...
                        flights
                            .iter()
                            .map(|r| r.flight_number2.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "aircraft_rotation_layover".into(),
                        flights
                            .iter()
                            .map(|r| r.aircraft_rotation_layover.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "operational_suffix2".into(),
                        flights
                            .iter()
                            .map(|r| r.operational_suffix2.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "flight_transit_layover".into(),
                        flights
                            .iter()
                            .map(|r| r.flight_transit_layover.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
    ...
    let ((c41, c42), (c43, c44)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "operating_airline_disclosure".into(),
                        flights
                            .iter()
                            .map(|r| r.operating_airline_disclosure.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "traffic_restriction_code".into(),
                        flights
                            .iter()
                            .map(|r| r.traffic_restriction_code.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "traffic_restriction_code_leg_overflow_indicator".into(),
                        flights
                            .iter()
                            .map(|r| r.traffic_restriction_code_leg_overflow_indicator.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "aircraft_configuration".into(),
                        flights
                            .iter()
                            .map(|r| r.aircraft_configuration.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
    ...
    let ((c45, c46), c47) = rayon::join(
        ...
        || {
            Column::new(
                "record_serial_number".into(),
                flights
                    .iter()
                    .map(|r| r.record_serial_number.as_ref())
                    .collect::<Vec<_>>(),
            )
        },
    );
    ...
    let ((s9, s10), (s11, s12)) = rayon::join(
        || {
            rayon::join(
                || {
                    Column::new(
                        "board_point_indicator".into(),
                        segments
                            .iter()
                            .map(|r| r.board_point_indicator.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
                    Column::new(
                        "off_point_indicator".into(),
                        segments
                            .iter()
                            .map(|r| r.off_point_indicator.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
            )
        ...
        || {
            rayon::join(
                || {
                    Column::new(
                        "data_element_identifier".into(),
                        segments
                            .iter()
                            .map(|r| r.data_element_identifier.as_ref())
                            .collect::<Vec<_>>(),
                    )
                },
                || {
        ...
        || {
            rayon::join(
                ...
                || {
                    Column::new(
                        "data".into(),
                        segments.iter().map(|r| r.data.as_ref()).collect::<Vec<_>>(),
                    )
                },
            )
        },
